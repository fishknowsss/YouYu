...
当前源码版本为 [`1.7.2`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.2)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.2 迭代说明](docs/releases/v1.7.2.md)。

## 1.7.2 修复重点

- 修复已安装 `1.7.0` 下载更新后点击“安装”仅退出、未真正执行安装的问题。新版使用受控提权启动器交接短时、用户与会话绑定的安装凭据；对旧客户端则保留经过严格校验的兼容恢复路径。授权被取消或安装器提前失败时，主程序会恢复为可重试状态。
- 每个公开更新通道的元数据都显式声明需要管理员提权，并在打包后验证，避免旧版 `electron-updater` 走到不稳定的跨 UAC 启动分支。
- 针对 Codex 的长连接重连，Mihomo 为 OpenAI 相关域名使用规则感知的稳定 DoH 解析与 TCP keep-alive；只会识别并重置确实无响应的 Codex/OpenAI TCP 连接，不会切换节点、改变手动/自动选择，或关闭其他应用连接。
- 软件更新的旋转提示紧贴在操作按钮左侧；手动点击“检查”立即进入动画状态，避免界面在主进程响应前看似卡住。
- 后台用户页不再展示无业务价值的长随机 ID；定向通知去掉重复的启用/停用控件，只保留通知级别与持续时间，资料区和操作行进一步对齐收紧。
- 保留 1.7.1 的独立通知窗口、前台/网络恢复即时同步和用户客户端版本盘点能力。
...
- 用户列表显示最近一次已认证请求所带的客户端版本及其上报时间。它用于设备盘点，不表示设备此刻一定在线；内部/无桌宠构建分别显示为 `-IN` / `-NO`。稳定用户 ID 仅用于后端归属与编辑，不在管理界面占用版面。
