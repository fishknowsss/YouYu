...
当前源码版本为 [`1.7.7`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.7)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.7 迭代说明](docs/releases/v1.7.7.md)。

## 1.7.7 更新重点

- 自动更新不再只确认安装器已经启动：独立监督器会等待安装完成、核对目标版本并确认新客户端主窗口真正就绪，失败时安全恢复并在首页提示。
- 后台全局默认允许用户“自行配置”，并支持逐用户跟随全局、允许或不允许；获允许用户在专业模式显式保存时建立单独配置，管理员重置后，下次同步重新跟随最新全局值。
- 首页保持简洁连接警告，导出的诊断文件则安全保留 Mihomo 的目标端口、协议、规则、进程、代理链和底层 dial 根因，并避免同类故障重复刷屏。
- 自动选点失败会恢复原 selector，后台健康恢复不会再覆盖更新的手动选择；身份切换和云配置未就绪时也不会退回另一用户或本机配置启动。
- 8 月 12 日纠正版修复了更新后启动期自动选点被后台健康恢复替换、反复触发启动失败与完整修复的问题；节点探测失败的安全细节也会进入诊断导出。
