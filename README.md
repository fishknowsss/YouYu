...
当前源码版本为 [`1.7.1`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.1)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.1 迭代说明](docs/releases/v1.7.1.md)。

## 1.7.1 迭代重点

- 定向通知改为独立的透明桌面窗口：标准版和无桌宠版固定显示在主显示器工作区右下；桌宠显示在底边时通知位于上方，贴在左右边时通知朝屏幕内侧显示，并持续避让桌宠与工作区边界。
- 通知卡片使用明确的提示/警告层级、可滚动长文本和“知道了”确认动作；确认后只隐藏当前设备的当前修订，后续新修订仍可送达。
- 远程配置常规每 3 分钟同步；主窗口重新获得焦点、托盘重新打开主窗口、网络恢复时会立即请求一次同步，12 秒内合并重复触发。它是拉取机制，不承诺实时推送。
- 后台用户列表显示“最近上报客户端版本”和上报时间。`1.7.1` 起内部通道标为 `-IN`、无桌宠通道标为 `-NO`；该信息来自用户设备的已认证请求，不能当作实时在线探针。
- 用户名编辑收紧为紧凑的资料与设备区；定向通知改为持续时间而非绝对到期时间，默认 10 分钟，以 5 分钟步进调整，最长 7 天。每次保存由 Worker 服务器时钟重新计时，并以请求 ID 安全重试。
- 已下载更新仍会继续发现更高版本，安装前会再次确认远端版本；更新重试不再把中间失败逐条写入诊断，更新组件与运行状态的视觉反馈保持统一。

## 界面预览

截图使用 `1.7.1` 的可重复安全演示数据。常规界面按 `900×600` 窗口采集；通知卡片按实际 `336×188` 独立窗口采集。订阅使用 `example.com`，出口 IP 使用 RFC 文档保留地址，节点、流量、测速、策略链和诊断内容均为虚构演示数据。
...
![YouYu 定向通知](docs/screenshots/notice-primary-display.png)
...
- 用户列表显示最近一次已认证请求所带的客户端版本及其上报时间。它用于设备盘点，不表示设备此刻一定在线；`1.7.1` 及以后内部/无桌宠构建分别显示为 `-IN` / `-NO`。
- 管理员可以向单个已登记用户发送一条纯文本通知。消息最长 500 个字符，只支持“提示”和“警告”，不接受 HTML、链接、命令或富内容，不能远程执行客户端操作。
...
- 直接运行 `team-builds/` 中的私有 `-in` / `-no` EXE 属于手动覆盖安装，不等同于一次自动更新验收；手动安装前应通过托盘正常退出 YouYu，避免外部应用在 Mihomo 停止而系统代理仍指向本地端口的短暂交接窗口。
