...
当前源码与最新公开发布版本为 [`1.6.0`](https://github.com/fishknowsss/YouYu/releases/tag/v1.6.0)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本次完整说明见 [v1.6.0 发布说明](docs/releases/v1.6.0.md)。

## 1.6.0 发布重点

- 专业模式设置页重排为订阅、三行主设置、诊断工具和软件更新，诊断状态、日志条数与“导出”动作在 `900×600` 下保持严格对齐。
- 诊断日志默认导出到用户“下载”文件夹；报告继续采用字段白名单和文本脱敏，最多包含当前会话 200 条记录。
- 后台登记、配置同步和流量上报改用 Cloudflare Worker 自定义域名与独立应用内直连会话；只有可恢复的连接故障才尝试正在运行的本地 Mihomo，不再被 `workers.dev`、异常系统 DNS 或失效本地代理单点卡住。
- 自动更新使用 GitHub `releases/latest/download` 通道入口，元数据检查和安装包下载都具备直连、应用内 DNS 刷新和本地代理回退。
- Electron 内置解析器、Happy Eyeballs 与 DNSPod、阿里云、Cloudflare 多入口 secure DNS 避开被污染的系统解析，并提升 IPv4/IPv6 异常时的成功率；不修改 Windows DNS、Hosts 或固定目标 IP。
- 双线路失败会在诊断中保留脱敏的 DNS、超时或连接错误码，不记录请求 URL、订阅、身份或请求体。
- 诊断与更新状态补充读屏播报，禁用按钮不会再显示可点击的悬停反馈。
- 本地只留存带私有订阅的 `-in`、`-no` 双包；公开三个更新通道全部为空内置订阅，私有包不会上传。

## 界面预览

设置页截图来自 `1.6.0`，其余截图来自 `1.5.6`，均为 `900×600` 开发预览。订阅使用 `example.com`，出口 IP 使用 RFC 文档保留地址，节点、流量、测速和诊断内容均为虚构演示数据。
...
设置页以订阅、三行主设置、诊断工具和软件更新组织规则来源、后台刷新、TUN、系统代理、DNS 增强、流量识别、同步、修复与日志导出。
