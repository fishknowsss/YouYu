...
- 后台已有姓名可直接重新登记并恢复该用户的远程配置、云端累计量和当日用量；同一安装环境重复登记保持原设备标识。
- 专业模式左下角版本号新增隐藏的重新登记入口；切换前结清旧用户，切换后清空旧本地状态并以目标用户云端累计量和当日用量继续计算。
- 修复专业模式左侧当前页面在悬停时被浅色背景覆盖的问题；选中项现在只会从品牌紫轻微加深，并保留清晰白字。
- 设置页“导出”和“检查”统一使用 16px、低字重的底部动作规格，右侧列宽与边缘保持一致。
- 软件更新行由 64px 收紧为默认 54px，与 46px 诊断行形成一个 8px 节奏级差；动态下载或长错误仍可自然增高。
- 私有手动分发目录改为扁平 `team-builds/`，每次本地三包构建成功后只保留当前版本的 `-in`、`-no` 两个 EXE，不再建立版本子目录。
- `.blockmap` 只保留在自动更新与公开归档链路，不再随手动安装包分发；私有目录仍由 Git 忽略和仓库卫生校验双重保护。

## 界面预览

设置页截图来自 `1.6.1`，其余截图来自 `1.5.6`，均为 `900×600` 开发预览。订阅使用 `example.com`，出口 IP 使用 RFC 文档保留地址，节点、流量、测速和诊断内容均为虚构演示数据。
