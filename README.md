...
当前源码版本为 [`1.6.10`](https://github.com/fishknowsss/YouYu/releases/tag/v1.6.10)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.6.10 迭代说明](docs/releases/v1.6.10.md)。

## 1.6.10 迭代重点

- 修复中文 Windows 将 `reg.exe` 本地码页错误输出按 UTF-8 解码后，缺失的可选代理值被误判为致命错误，导致 1.6.9 启动后立即回滚的问题；代理读取改为按注册表结构判断，不再匹配本地化错误文本。
- 修复提升助手把 `powershell.exe` 错解析到 YouYu 安装目录、进而伪报“需要管理员权限”的问题；完整修复在关键阶段成功后会继续恢复原有运行意图，附加网络清理失败不再把应用永久留在停止状态。
- 小白模式的错误与更新提示统一堆叠在右下角，左下七击专业模式入口不再被遮挡；专业诊断以清晰摘要替代原始注册表命令和乱码，正常回滚产生的 `SIGTERM` 不再显示成 Mihomo 崩溃。
- `1.6.10` 为仍停留在 `1.6.8` 及更早版本的客户端保留严格受限的一次性更新桥；1.6.9 及以后版本仍必须使用绑定用户、会话和短时凭据的认证交接。
...
截图沿用 `1.6.9` 的安全演示模式与 `900×600` 默认窗口；`1.6.10` 未改变这些主界面内容。订阅使用 `example.com`，出口 IP 使用 RFC 文档保留地址，节点、流量、测速、策略链和诊断内容均为虚构演示数据。
