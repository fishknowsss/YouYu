...
function createMihomoApiClient(options) {
  ...
  return {
    async listNodes() {
      ...
      const current = selector?.now ?? "自动选择";
      return all.map((name) => {
        const history = proxies[name]?.history ?? [];
        const delay = history.findLast((item) => typeof item.delay === "number")?.delay;
        return {
          name,
          delay,
          active: name === current
    ...
    async selectNode(name) {
      await request(`/proxies/${encodeURIComponent(selectorName)}`, {
        method: "PUT",
...
function buildMihomoConfig(input) {
  const config = {
    "mixed-port": 7890,
    "allow-lan": false,
    mode: "rule",
...
async function waitForController(secret) {
...
function createMihomoRuntime(options) {
  let child = null;
  ...
  return {
    async start() {
      ...
      const spawnProcess = options.spawnProcess ?? ((binaryPath, args) => spawn(binaryPath, args, {
        windowsHide: true,
        stdio: "ignore"
      }));
      child = spawnProcess(options.binaryPath, ["-d", workDir, "-f", configPath]);
      child.once("exit", () => {
        child = null;
      });
      await (options.waitForReady ?? waitForController)(settings.controllerSecret);
    },
    async stop() {
      const current = child;
...
function parseServer(output) {
  const match = output.match(/ProxyServer\s+REG_SZ\s+(.+)/i);
  return match?.[1]?.trim() ?? "";
}
function createSystemProxyAdapter(options = {}) {
  const platform = options.platform ?? process.platform;
  const runCommand = options.runCommand ?? defaultRunCommand;
  let previous = null;
  async function queryPrevious() {
    const [enabledOutput, serverOutput] = await Promise.all([
      runCommand(`reg query "${internetSettingsKey}" /v ProxyEnable`),
      runCommand(`reg query "${internetSettingsKey}" /v ProxyServer`).catch(() => "")
    ]);
    return {
  ...
  async function setProxy(enabled, server) {
    await runCommand(
      `reg add "${internetSettingsKey}" /v ProxyEnable /t REG_DWORD /d ${enabled ? 1 : 0} /f`
    );
    if (server !== void 0) {
      await runCommand(`reg add "${internetSettingsKey}" /v ProxyServer /t REG_SZ /d "${server}" /f`);
    }
    await runCommand(
      `powershell -NoProfile -ExecutionPolicy Bypass -Command "$signature = '[DllImport("wininet.dll", SetLastError=true)] public static extern bool InternetSetOption(IntPtr hInternet, int dwOption, IntPtr lpBuffer, int dwBufferLength);'; $type = Add-Type -MemberDefinition $signature -Name WinInet -Namespace Native -PassThru; $type::InternetSetOption([IntPtr]::Zero, 39, [IntPtr]::Zero, 0); $type::InternetSetOption([IntPtr]::Zero, 37, [IntPtr]::Zero, 0)"`
    );
  }
  return {
    async enable() {
    ...
    async restore() {
      ...
      await setProxy(previous.enabled, previous.server || void 0);
    },
    async repair() {
      if (platform !== "win32") return;
      await setProxy(false);
...
`, "utf8");
  ...
  normalize(value) {
    return {
      ...
      controllerSecret: typeof value.controllerSecret === "string" && value.controllerSecret.length >= 16 ? value.controllerSecret : this.createSecret()
    };
  }
  createDefaults() {
    return {
...
async function saveSubscriptionSettings(deps, subscriptionUrl) {
  await deps.settingsStore.update({ subscriptionUrl: subscriptionUrl.trim() });
  if (deps.lifecycle.getStatus() === "running") {
    await deps.lifecycle.restart();
  }
  return deps.createSnapshot();
}
async function updateSubscriptionNodes(deps) {
...
const mihomoBinaryPath = isDev ? join(process.cwd(), "resources/mihomo/win-x64/mihomo.exe") : join(process.resourcesPath, "mihomo/win-x64/mihomo.exe");
const windowIconPath = isDev ? join(process.cwd(), "build/icon.png") : join(process.resourcesPath, "assets/icon.png");
const mihomoRuntime = process.platform === "win32" ? createMihomoRuntime({
...
async function createSnapshot() {
  ...
  const nodes = lifecycle.getStatus() === "running" ? await mihomoApi.listNodes().catch(() => []) : [];
  const currentNode = nodes.find((node) => node.active)?.name ?? "自动选择";
  return {
...
function registerIpc() {
  ...
  ipcMain.handle(ipcChannels.selectNode, async (_event, name) => {
    const settings = await settingsStore.read();
    const mihomoApi = createMihomoApiClient({ secret: settings.controllerSecret });
