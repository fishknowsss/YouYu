...
当前源码版本为 [`1.7.0`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.0)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.0 迭代说明](docs/releases/v1.7.0.md)。

## 1.7.0 迭代重点

- 已下载更新不再锁死当前会话：周期检查会继续发现更高版本，点击安装前还会执行新鲜度检查；同版继续静默安装，发现新版时阻止旧包交接并切换下载，检查失败时保留原下载供重试。
- 更新网络重试改为动态读取当前 Mihomo 代理，自动检查不再把每次中间失败写进诊断；同时过滤无意义的 `CLIXML` 声明、显示真实失败站点，并在计划任务不可写时沿用安全兼容的开机启动路径。
- 小白模式与设置页的更新旋转指示统一放在整个组件左侧，设置页“检查”与“保存”使用相同主按钮样式；专业模式首页运行状态增加成功、失败与停止对应的语义边框。
- 流量后台支持管理员纠正用户名并审计保存；旧名保留为同一用户的别名，签名远程配置只向匹配身份的客户端同步权威显示名，不改变设备、流量和订阅归属。
- 后台支持向指定用户发送可过期、可确认、按修订重发的纯文本提示或警告；HTML 标签只会原样显示，不支持命令或富内容，不能远程执行客户端操作。
- 1.6.8 及更早客户端的一次性更新桥唯一目标迁移到 1.7.0，源版本上限与身份校验不放宽；后台功能上线前必须完成 D1 迁移检查、显式应用、复核和 Worker 部署门禁。
...
截图沿用 `1.6.9` 的安全演示数据与 `900×600` 默认窗口。`1.7.0` 调整了更新组件的旋转指示位置，并为专业模式运行状态补充语义边框；下列静态截图可能未展示这些瞬时或局部状态。订阅使用 `example.com`，出口 IP 使用 RFC 文档保留地址，节点、流量、测速、策略链和诊断内容均为虚构演示数据。
