...
- When updater or installer PowerShell changes, run both the focused real Windows PowerShell 5.1 tests and the full local `npm test` suite with Vitest's default file parallelism. On Actions, keep default file parallelism for the general suite, then run the heavy updater process-boundary file as a separate no-retry phase with its original per-test timeouts. Any production Node, NSIS, or other intermediate process that starts canonical Windows PowerShell 5.1 must remove every case variant of inherited `PSModulePath` and `PSModuleAnalysisCachePath` before the first process starts, so 5.1 rebuilds its native module path and uses its native cache. Actions tests must start through `cmd`, explicitly discard inherited module paths, and may let repeated real-script fixtures reuse only an existing absolute, non-`NUL` analysis-cache file supplied by the runner; unit and spawn-contract tests plus a lightweight poisoned-parent real child must prove production removes both settings, while a cached poisoned-module-path test verifies 5.1 resolves inbox modules from `$PSHOME`. Do not hide cross-version module resolution failures with hard-coded module paths, disabling or custom-prewarming caches, retries, longer timeouts, or `--no-file-parallelism`; reduce duplicate fixture startup or isolate the process boundary, then confirm the exact pushed commit on a fresh Actions runner.
