...
当前源码版本为 [`1.7.4`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.4)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.4 迭代说明](docs/releases/v1.7.4.md)。

## 1.7.4 更新重点

- 自动更新发现直连 GitHub CDN 长时间停滞，或速度低到预计需要异常久且本地代理已经可用时，会停止当前尝试并自动切换到本地代理继续下载。
- 小白模式和专业模式都会显示“线路不稳定，已自动切换重试”，下载过程不会仅因换路而中止。
- 自动换路保持安全边界：证书、HTTP 状态、签名和文件完整性错误不会被当作普通网络波动重试。
- 发布流程新增 CDN 路由预检与统一远端校验器，固定检查精确 11 个公开资产、三个更新通道和完整 SHA256 清单，避免发布工具误走不可用直连。
