...
本地可使用 `npm run dist:win:in` 或 `npm run dist:win:no` 构建带本机内部订阅的安装包。这类产物仅供本机或内部临时使用，不能上传到 GitHub Release、Actions artifact 或其他公开下载位置。
...
在设置页执行“修复”，让应用清理系统代理、WinHTTP 代理和 DNS 缓存。如果仍未恢复，再检查其他代理软件、VPN、网卡、路由器和系统网络设置。
