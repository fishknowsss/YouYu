...
export function createUpdateInstallerLauncherScript(): string {
  return [
    ...
    `$env:${updateElevatedInstallerPayloadEnvironment} = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($elevatedPayloadJson))`,
    "$elevatedWrapperArgumentLine = '-NoProfile -NonInteractive -ExecutionPolicy Bypass -File ' + (ConvertTo-WindowsCommandLineArgument $elevatedInstallerWrapperPath $true)",
    `try { $started = Start-Process -FilePath ([IO.Path]::Combine($PSHOME, 'powershell.exe')) -ArgumentList $elevatedWrapperArgumentLine -Verb RunAs -WindowStyle Hidden -PassThru } finally { Remove-Item Env:${updateElevatedInstallerPayloadEnvironment} -ErrorAction SilentlyContinue }`,
