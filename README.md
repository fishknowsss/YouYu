...
当前源码版本为 [`1.7.3`](https://github.com/fishknowsss/YouYu/releases/tag/v1.7.3)。公开安装包与更新文件见 [GitHub Releases](https://github.com/fishknowsss/YouYu/releases/latest)，版本演进见 [CHANGELOG](CHANGELOG.md)，本版完整说明见 [v1.7.3 迭代说明](docs/releases/v1.7.3.md)。

## 1.7.3 更新重点

- 自动模式默认优先日本节点；同一地区内先比较双站点探测健康度，再比较延迟，并通过当前代理出口校验实际国家代码。
- 日本节点均不可用时不会中断日常工作，而是回退到其他健康低延迟节点，并在主界面或系统通知中说明已切换到的地区。
- 小白模式启动、专业模式切回“自动”、启动恢复、节点健康恢复和订阅更新后都执行同一套地区选点策略；手动模式保持用户选择，不被后台静默改写。
- 后台新增全局“优先地区/地区不可用”设置，并允许每个用户单独覆盖。默认是“日本优先 + 自动切换”，也可为指定用户设置严格地区或最低延迟。
- 数据库迁移会为既有全局配置补上日本优先默认值；旧客户端缓存没有新字段时也按同一默认值运行，升级后无需重新安装或手动清理设置。
